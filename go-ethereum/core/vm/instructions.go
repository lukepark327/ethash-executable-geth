...
func opEthash(pc *uint64, interpreter *EVMInterpreter, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	...
	// _, _, _, _, _ = num, mixdigest, truncated, difficulty, nonce

	// Recompute the digest and PoW value and verify against the header
	number := num.Uint64()

	cacheCache := ethash.New(ethash.Config{
		CacheDir:       "otherEthash",
		CachesInMem:    2,
		CachesOnDisk:   3,
		DatasetsInMem:  1,
		DatasetsOnDisk: 2,
	}, nil, false).Cachecache(number)

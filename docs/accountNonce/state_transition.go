...
func (st *StateTransition) TransitionDb() (ret []byte, usedGas uint64, failed bool, err error) {
	...
	if contractCreation {
		...
		currentNonce := st.state.GetNonce(sender.Address())
		base := st.evm.BlockNumber.Uint64() * MaxTransactionPerBlock
		if int64(currentNonce-base) < 0 {
			log.Info("RESET NONCE", "base", base, "nonce", currentNonce)
			st.state.SetNonce(msg.From(), base)
		} else {
			log.Info("NONCE++", "base", base, "nonce", currentNonce)
			st.state.SetNonce(msg.From(), currentNonce+1)
		}
